...
const WEATHER_API_KEYS = ["db48eb8771c490a152697832b08d7d8a", "1128cc327ca574b3efd0a91663de5e00"];
...
function weather() {
	...
	function weatherRequest(arg) {
		...
		function request(lat, long, lang) {

			var url = "https://api.darksky.net/forecast/"
			+ WEATHER_API_KEYS[0] + "/"
			+ lat + ","
			+ long + "?exclude=hourly,alerts,flags&units=auto&lang="
			+ (lang ? lang : "en");
		...
		navigator.geolocation.getCurrentPosition((pos) => {
			...
			chrome.storage.local.set({"weather_lat": pos.coords.latitude});
			chrome.storage.local.set({"weather_long": pos.coords.longitude});
	...
	if (lastState) console.log(lastState);
